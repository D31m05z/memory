...
void draw_menu() {
    ...
    al_draw_textf(i_font, al_map_rgb(0xad, 0xd8, 0xe6), 100, 300, 0, "<M>     - jatek mod : %s",
                  i_game_type ? "EXTREM" : "NORMAL");
...
void draw_screen() {
    ...
    if (i_found == (i_game_type ? 7 : 8)) {
        if (i_players[0].score > i_players[1].score)
            al_draw_textf(i_font, al_map_rgb(0xad, 0xd8, 0xe6), i_width / 2 - 100, i_height / 2, 0, "Nyert: %s !",
                          i_players[0].name);
